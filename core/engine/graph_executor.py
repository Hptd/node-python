...
def topological_sort(nodes: List[SimpleNodeItem]) -> List[SimpleNodeItem]:
    ...
    支持 SimpleNodeItem 和 LoopNodeItem 混合列表。
    """
    from ..graphics.loop_node_item import LoopNodeItem
    
    # 过滤掉非 SimpleNodeItem 和 LoopNodeItem
    valid_nodes = [n for n in nodes if isinstance(n, (SimpleNodeItem, LoopNodeItem))]
    
    if not valid_nodes:
        return []
    
    in_degree = {node: 0 for node in valid_nodes}
            ...
            if port.connections:
                for conn in port.connections:
                    if conn.start_port:
                        ...
                        # 只统计来自 SimpleNodeItem 或 LoopNodeItem 的依赖
                        if isinstance(source_node, (SimpleNodeItem, LoopNodeItem)) and source_node in valid_nodes:
                            in_degree[node] += 1

    queue = [node for node in valid_nodes if in_degree[node] == 0]
    sorted_nodes = []
        ...
        for port in node.output_ports:
            for conn in port.connections:
                if conn.end_port:
                    ...
                    # 只处理在排序列表中的节点
                    if isinstance(target_node, (SimpleNodeItem, LoopNodeItem)) and target_node in in_degree:
...
def execute_graph_with_loops(
    ...
    """
    from utils.console_stream import colored_print
    from .loop_executor import execute_graph_with_loops as loop_executor_func

    try:
        return loop_executor_func(nodes, loop_nodes)
    except Exception as e:
        colored_print(f"\n运行出错：{e}", "error")
        import traceback
        colored_print(traceback.format_exc(), "error")
