...
def _build_loop_execution_script(
    ...
    script_parts.append("# 添加项目根目录到路径")
    script_parts.append("python_exe_dir = os.path.dirname(os.path.abspath(sys.executable))")
    script_parts.append("possible_paths = [")
    script_parts.append("    os.environ.get('NODE_PYTHON_PROJECT_DIR', ''),")
    script_parts.append("    python_exe_dir,")
    script_parts.append("    os.path.dirname(python_exe_dir),")
    script_parts.append("]")
    script_parts.append("for path in possible_paths:")
    script_parts.append("    if path and os.path.isdir(path) and os.path.isdir(os.path.join(path, 'utils')):")
    script_parts.append("        sys.path.insert(0, path)")
