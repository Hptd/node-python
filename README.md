...
首次运行前需要初始化 `python_embedded` 环境：
1. 运行 `python -m utils.setup_embedded_python install`（开发环境）
2. 或通过程序工具栏的"🔧 初始化环境"按钮
3. 程序会自动下载嵌入式Python并安装pip
...
1. **exe文件小巧**：主程序仅包含GUI，体积小
2. **环境隔离**：节点执行在独立进程中，不影响主程序稳定性
3. **灵活扩展**：用户可在无Python环境的电脑上安装任何第三方库
4. **易于维护**：Python环境可单独更新或重置
5. **统一执行**：所有节点（内置/自定义）使用相同环境，避免兼容性问题
...
用户首次运行
    ↓
检查 python_embedded/python.exe 是否存在
    ↓
不存在 → 提示初始化环境
    ↓
下载嵌入式Python (约10MB)
