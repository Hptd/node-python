...
- **区间循环节点（Range Loop）**：按整数范围和步长进行循环迭代
  ...
  - 输出端口：迭代值（当前整数值）、汇总结果（所有结果列表）
  - 支持外部节点输入控制循环参数
- **List 循环节点（List Loop）**：遍历列表中的每个元素
  - 输入端口：列表数据（JSON 格式或 list 类型）
  - 输出端口：迭代值（当前元素）、汇总结果（所有结果列表）
  - 支持外部节点输入控制列表数据
- **连接模式**：节点连接到循环节点输出端口
  - 连接到"迭代值"：每次迭代执行
  - 连接到"汇总结果"：循环完成后执行一次
...
│   │   └── ...
│   └── engine/               # 执行引擎
│       ├── graph_executor.py # 图表执行器
│       └── loop_executor.py # 循环执行器
├── ui/                        # 用户界面
│   ├── dialogs/              # 对话框
│   ├── widgets/              # 自定义控件
│   │   └── node_search_menu.py  # 节点搜索菜单
│   └── main_window.py        # 主窗口
├── storage/                   # 存储模块
└── utils/                     # 工具函数
    ├── constants.py          # 常量定义
...
- **循环节点**：在 `core/graphics/loop_node_item.py` 和 `core/engine/loop_executor.py` 中扩展
